...
|---|---|---|
| **Check calico-node DaemonSet** | [`checkCalicoDaemonSet()`](../internal/controller/cni/controller.go#L160-L205) | Reads DaemonSet `status.desiredNumberScheduled` vs `status.numberReady`. If mismatched, lists pods with label `k8s-app=calico-node` and checks `isPodReady()`. Unready pod names stored in `signalCalicoNodeUnready`. |
| **Check Stuck Workload Pods** | [`checkStuckPods()`](../internal/controller/cni/controller.go#L207-L268) | Scans all cluster pods in `PodPending` phase with empty `PodIP`. Checks if `time.Since(creationTimestamp) > StuckPodThresholdSeconds` (default 60s). |
| **Inspect Pod Warning Events** | [`findCNIEvent()`](../internal/controller/cni/controller.go#L270-L305) | Queries Kubernetes `EventList` for `FailedCreatePodSandBox`. Performs case-insensitive matching for `"failed to setup network"`, `"no ips available in pools"`, `"failed to allocate"`. Stores in `signalStuckPods`. |
| **Check Calico IPAM Blocks & IPPools** | [`checkIPAMUsage()`](../internal/controller/cni/controller.go#L307-L365) | Queries Calico's `IPPoolList` to detect `spec.disabled == true`. Queries `IPAMBlockList` to calculate `used / total` IP slots per block CIDR. Stores in `signalIPAMExhaustedPools` and `signalDisabledIPPools`. |

---

### Step 2: `Evaluate()` Phase — Heuristic Decision Engine
In `Evaluate(ctx context.Context, checkResult *module.CheckResult)`:

```go
// 1. Calico-node crash is highest severity
...
// 2. Disabled IPPools while pods are waiting for IPs
...
// 3. IPAM address exhaustion
if len(exhaustedPools) > 0 || hasIPAMStuck(stuckPods) {
    return &module.EvalResult{
        IsHealthy: false, NeedsRemediation: true, Severity: "critical",
        Reason: fmt.Sprintf("IPAM exhaustion: %d block(s) at 100%%...", len(exhaustedPools)),
    }, nil
}
```
...
### Step 3: `Remediate()` Phase — Surgical Automated Recovery
In `Remediate(ctx context.Context, evalResult *module.EvalResult)`:

1. **Restart Unready CNI Agents:**
   ```go
   ...
   ```
   *Why this works:* Deleting the crashing `calico-node` pod forces the DaemonSet controller to launch a clean container instance with fresh sockets and route tables.

2. **Re-Enable Disabled IPPools:**
   ```go
   patch := item.DeepCopy()
   unstructured.SetNestedField(patch.Object, false, "spec", "disabled")
   _ = m.Client.Patch(ctx, patch, client.MergeFrom(&item))
   ```
   *Why this works:* Re-enabling the pool immediately restores IP allocation capability to the CNI without requiring human manual intervention.

3. **Evict Stuck Workload Pods:**
   ```go
   ...
   ```
   *Why this works:* Workload pods stuck in `ContainerCreating` will never recover on their own. Evicting them forces the ReplicaSet controller to recreate them, acquiring fresh IP addresses immediately from the restored CNI/IPAM.
