...
\setlist{nosep, leftmargin=1.5em, labelindent=0pt, align=left}
...
\begin{tabular}{ccc}
\textbf{Arpan Gupta} &
\textbf{Buha Deep Maheshbhai} &
\textbf{Param Tanna} \\[-1mm]
24110051@iitgn.ac.in &
24110082@iitgn.ac.in &
param.tanna@iitgn.ac.in \\[4mm]

\textbf{Ramji Purwar} &
\textbf{Rathod Yuvraj Rajubhai} &
\textbf{Solanki Viraj Rajeshbhai} \\[-1mm]
ramji.purwar@iitgn.ac.in &
yuvraj.rathod@iitgn.ac.in &
...
Framed in the vocabulary of computer networks, the faults we target are not operational curiosities but failures spread across the entire protocol stack. At the \emph{link layer}, each pod attaches to its host through a virtual Ethernet (veth) pair and resolves on-node addresses over ARP, so a downed veth or corrupted bridge breaks L2 frame delivery while the pod process stays alive. At the \emph{network layer}, the CNI's IP Address Management (IPAM) assigns every pod an IP, and cross-node traffic rides an overlay in which Calico encapsulates the original pod-to-pod packet inside an outer IP header using IP-in-IP or VXLAN; an exhausted IPAM pool or a dropped tunnel interface (\texttt{tunl0}) thus manifests as a pure L3 addressing and routing failure. At the \emph{transport layer}, reachability is exercised with ICMP echo and TCP connection probes-the same primitives behind \texttt{ping} and a port check. At the \emph{application layer}, service discovery depends on DNS: pods resolve names by sending UDP queries to CoreDNS on port~53, which answers cluster-internal names authoritatively and forwards external ones to an upstream resolver. NetworkPolicy enforcement is, at bottom, stateful packet filtering, with Calico's Felix agent compiling each policy into Linux \texttt{netfilter} (\texttt{iptables}/\texttt{ipset}) rules that match on L3/L4 header fields. Our operator is therefore best understood not as a container-management add-on but as a system that watches and repairs faults along the full path a packet travels-from the veth interface, through IP routing and the overlay tunnel, to DNS resolution at the top of the stack.
...
This module guards the link- and network-layer plumbing every pod depends on before it can send a single packet: the veth pair that connects it to the host, the IPAM allocation that gives it an L3 address, and the BGP/overlay routing mesh that carries its traffic between nodes.
...
DNS is the application-layer directory of the cluster: every service call begins with a name lookup. This module keeps that resolution path healthy, watching both the internal query service (UDP to CoreDNS on port~53) and the upstream forwarding used to resolve external names.
...
A NetworkPolicy is, in networking terms, a stateful packet-filter rule matched on L3/L4 header fields (source/destination IP, port, protocol). Kubernetes only stores this intent; the CNI must program it into the kernel firewall. This module verifies that the enforcement path is actually alive, so that the rules a packet is checked against reflect the declared policy.
...
This module works at the network and transport layers, actively testing whether packets can actually traverse the veth, bridge, and overlay-tunnel path between pods-something readiness probes, which only check a local process, never verify.
...
This yields exactly $2N$ inter-node probes, achieving $O(N)$ scalability versus the $O(N^2)$ cost of full-mesh probing. Each probe is a lightweight ICMP echo or TCP connection attempt to the target pod's IP address, so a failed probe directly signals loss of L3/L4 reachability along that specific path. Each node additionally undergoes an $O(1)$ intra-node canary validation to verify local veth, bridge, and iptables FORWARD chain integrity.
...
\setlength{\tabcolsep}{3pt}
\footnotesize
\begin{tabular}{@{}c l p{3.2cm} c@{}}
\toprule
\textbf{ID} & \textbf{Module} & \textbf{Fault Injected} & \textbf{Healed} \\
\midrule
E1 & CNI & calico-node agent crash & \checkmark \\
E2 & CNI & IPAM pool exhaustion & \checkmark \\
E3 & CNI & IPPool disabled & \checkmark \\
E4 & CoreDNS & Scale replicas to zero & \checkmark \\
E5 & CoreDNS & CPU throttle ($\leq$5\,m) & \checkmark \\
E6 & CoreDNS & Corrupted upstream forward & \checkmark \\
E7 & CoreDNS & Pod CrashLoopBackOff & \checkmark \\
E8 & NetPolicy & Felix agent crash & \checkmark \\
E9 & PodConn & veth interface down & \checkmark \\
E10 & PodConn & IPIP tunnel crash & \checkmark \\
E11 & PodConn & iptables FORWARD DROP & \checkmark \\
E12 & PodConn & Inter-node overlay break & \checkmark \\
\bottomrule
\end{tabular}
\end{table}
